...
void kill_gateway(void)
{
	...
	close(cd);
	char *kill_char = (char *)malloc(50);
	memset(kill_char,0,50);
	memcpy(kill_char,"kill ",5);
	int_to_str((int)main_pid,kill_char+5);
...
void get_mac(char *MyMac)
{
    ...
    strcpy(req.ifr_name, "wlan0");    ///Currently, only get wlan0
...
void get_sn_from_server(cJSON *root)
{
	...
	if(ret->valueint==0)
	{
		...
		if(sn_old_or_new->valueint==0)//new sn
		{
			...
			cJSON_AddStringToObject(notice_data,"type","3");
		...
		else if(sn_old_or_new->valueint==1)//old sn
		{
			...
			cJSON *sn_mac_root = cJSON_CreateObject();
			...
			fsync(sn_mac_fd);
			close(sn_mac_fd);
			cJSON_Delete(sn_mac_root);
			sn_mac_root = NULL;
			free(sn_mac_root_char);
			sn_mac_root_char = NULL;
